...
namespace Fusion.XR
{
    public class FusionXRHand : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (controlType == ControlInputType.XR)
            {
                Debug.Log("XR Controls loaded...");
            ...
            if (controlType == ControlInputType.PogoPins)
            {
                Debug.Log("Pogo Pin Controls loaded...");
                if (hand == Hand.Left)
                {
                    gripAction = SteamVR_Actions.BioSpectra.gripActionLeft;
                    triggerAction = SteamVR_Actions.BioSpectra.triggerActionLeft;
                }
                else
                {
                    gripAction = SteamVR_Actions.BioSpectra.gripActionRight;
                    triggerAction = SteamVR_Actions.BioSpectra.triggerActionRight;
                }
                gripAction.onStateDown += OnGrabbed;
                gripAction.onStateUp += OnLetGo;
                triggerAction.onStateDown += OnPinched;
                triggerAction.onStateUp += OnPinchedCancelled;
            }
        ...
        private void OnGrabbed(SteamVR_Action_Boolean action, SteamVR_Input_Sources source)
        {
            //Debug.Log($"Action: {action.fullPath} |||| Source: {source}");
            OnGrabStart?.Invoke();
            GrabObject();
        }
        ...
        private void OnLetGo(SteamVR_Action_Boolean action, SteamVR_Input_Sources source)
        {
            //Debug.Log($"Action: {action} |||| Source: {source}");
            OnGrabEnd?.Invoke();
            Release();
        }
        ...
        private void OnPinched(SteamVR_Action_Boolean action, SteamVR_Input_Sources source)
        {
            OnPinchStart?.Invoke();
        }
        ...
        private void OnPinchedCancelled(SteamVR_Action_Boolean action, SteamVR_Input_Sources source)
        {
            OnPinchEnd?.Invoke();
        }
